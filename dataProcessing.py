...
def makeData(sp):
    ...
    for genre in set(genres):
        genreDict[genre.replace(' ', '-')] = genres.count(genre)
...
def genRecommendations(df, genreDict):
    ...
    for index, row in spotify_df.iterrows():
        spotify_df.at[index, 'sim'] =  cosine_similarity(np.array(row[feature_cols]).reshape(1,-1), weightedVector.reshape(1,-1)) + spotify_df.at[index, 'sim']*popFactor
        try:
